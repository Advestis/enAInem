...
class EnAInem(BaseEstimator):
    ...
    def generate_tensor(self, B: list[NDArray]) -> NDArray:
        """
        Reconstruct the tensor from a list of basis vectors.

        Parameters
        ----------
        B : list of NDArray
            Basis vectors in each dimension of the tensor.

        Returns
        -------
        NDArray
            The reconstructed tensor with the same shape as the original input.
        """
        return _generate_tensor(B)
